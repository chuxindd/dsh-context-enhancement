...
import { frameProjection } from './prompt.ts'
...
export type FoldedBatch =
  | { readonly kind: 'empty' }
  | { readonly kind: 'infeasible'; readonly frameBytes: number; readonly maxInputBytes: number }
  | { readonly kind: 'window'; readonly window: FoldedBatchWindow }
 ...
 */
export function foldBatchWindow(
  events: readonly { readonly seq: number; readonly type: string; readonly data: unknown }[],
  base: TaskStateStable | null,
  cursor: number,
  windowEndSeq: number,
  budget: BatchWindowBudget,
): FoldedBatch {
  const includedSeqs: number[] = []
  const projected: TaskStateFilteredEvent[] = []
  const truncation: TaskStateTruncationRecord[] = []
  ...
  for (const event of events) {
    ...
    const candidate: TaskStateFilteredEvent = {
