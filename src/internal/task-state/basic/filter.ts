...
export const DEFAULT_FILTER_FIELD_LIMITS: Readonly<FilterFieldLimits> = Object.freeze({
...
function toolResultText(message: unknown): string {
  ...
  const block = (content as readonly unknown[])[0]
...
function isMeaningfulLeaf(value: string): boolean {
...
function renderLeaves(leaves: readonly BoundedLeaf[]): string {
...
function projectArguments(
  ...
  wholeBytes: number,
  records: FilterTruncation[],
): { readonly kind: 'json'; readonly leaves: readonly BoundedLeaf[] } | { readonly kind: 'raw'; readonly text: string } {
  const raw = typeof argumentsRaw === 'string' ? argumentsRaw.trim() : ''
  if (raw.length === 0) return { kind: 'raw', text: '' }
  let parsed: unknown
  try {
...
function directHumanKind(message: unknown): 'user' | 'goal' | undefined {
  if (!isRecord(message)) return undefined
  const source = message['source']
  if (!isRecord(source)) return undefined
  const kind = source['kind']
  if (kind !== 'user' && kind !== 'goal') return undefined
  return kind
}
...
function projectTurnEndReason(reason: unknown): unknown {
...
function projectEvent(
  ...
  switch (event.type) {
    ...
    case 'assistant/message': {
      if (isRecord(data) && data['interrupted'] === true) return undefined
    ...
    case 'tool/result': {
      ...
      const first = Array.isArray(content) ? (content as readonly unknown[])[0] : undefined
      ...
      if (result.text === undefined && result.meta === undefined && result.error === undefined) return undefined
      return result
    }
    case 'turn/end': {
      if (!isRecord(data)) return undefined
    ...
    case 'request/header': {
      ...
      if (reason !== 'initial' && reason !== 'resume' && reason !== 'change') return undefined
    ...
    case 'command/run': {
      ...
      if (!LIFECYCLE_COMMANDS.has(name)) return undefined
...
const NEVER_ELIGIBLE: ReadonlySet<string> = new Set([
...
export function filterEvent(
  event: RawSessionEvent,
  limits: FilterFieldLimits = DEFAULT_FILTER_FIELD_LIMITS,
): FilterResult | null {
  if (!isEligibleType(event.type)) return null
  const records: FilterTruncation[] = []
