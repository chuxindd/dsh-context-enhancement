...
export function resolveInjectionBudget(config: TaskStatePromptConfig): ResolvedInjectionBudget {
 ...
 */
export function estimateSlotTokens(
  text: string,
...
function hasAuthorityContent(stable: TaskStateStable): boolean {
 ...
 */
function goalLines(stable: TaskStateStable): string[] {
  ...
  if (view.goalId !== undefined) identity.push(`goal ${view.goalId}`)
  if (view.goalRevision !== undefined) identity.push(`revision ${view.goalRevision}`)
  if (view.phase !== undefined) identity.push(`phase ${view.phase}`)
 ...
 */
function todoLines(stable: TaskStateStable): string[] {
 ...
 */
function buildLines(stable: TaskStateStable, isStale: boolean): {
  readonly lines: string[]
  readonly authoritativeLineCount: number
} {
  const lines = [
...
export interface RenderTaskStateSlotOptions {
  ...
  readonly meter?: { estimateMessage(message: Message): number } | undefined
}
 ...
 */
export function renderTaskStateSlot(
  stable: TaskStateStable,
  options: RenderTaskStateSlotOptions,
): RenderTaskStateSlotResult {
  ...
  const staleness: TaskStateStaleness = isStale ? 'stale' : 'fresh'
  ...
  if (
    encoder.encode(minTruncatedCandidate).byteLength > maxBytes
  ...
  for (let i = authoritativeLineCount; i < lines.length; i += 1) {
    ...
    if (
      encoder.encode(withMarker).byteLength <= maxBytes
