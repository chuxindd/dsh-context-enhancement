...
function closedToolStep(): Session {
  const session = Session.create(SessionId('ported-closed-tool-step'))
...
describe('tool-pairing boundaries (ported)', () => {
  it('classifies closed and open single-call steps', () => {
    ...
    const open = Session.create(SessionId('ported-open-tool-step'))
  ...
  it('requires every result from a multiple-call assistant message', () => {
    const session = Session.create(SessionId('ported-multiple-calls'))
    session.append('assistant/message', {
      turn: 1,
      step: 1,
      message: createMessage({
  ...
  it('keeps neutral nodes inside an open pair unbalanced and free nodes balanced', () => {
    const midStep = Session.create(SessionId('ported-neutral-mid-step'))
    ...
    const free = Session.create(SessionId('ported-neutral-free'))
...
describe('tool-pairing surface identity (ported)', () => {
...
describe('tool-pairing cache refresh (ported)', () => {
...
describe('tool-pairing corrupt surfaces (ported)', () => {
  it('retries an orphan result in an appended tail without committing partial cache state', () => {
    const session = Session.create(SessionId('ported-orphan-tail'))
    session.append('user/message', createUserMessage({
