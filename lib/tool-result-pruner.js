import { m as Schema, s as freezeMessage } from "./lib-UkEFLxaM.js";
import { c as DEFAULTS, d as resolveConfig, l as PRUNE_MARKER, n as reductionProvenance, r as shadowPriceWithProvenance, s as toolResultTextLength, u as codePointLength } from "./source-index-BiRfSNPU.js";
	...
	reduceMessageBlocks(blocks, removedStart, removedEnd, state) {
		...
		for (const block of blocks) {
			if (block.type === "tool-result") {
				pruned.push({
					...block,
					content: this.reduceMessageBlocks(block.content, removedStart, removedEnd, state)
				});
				continue;
			}
			if (block.type !== "text") {
			...
			const marker = blockStart < removedEnd && blockEnd > removedStart && !state.markerInserted ? PRUNE_MARKER : "";
	...
	*/
	pruneSession(session, options) {
		...
		for (const { seq, event, ordinaryEligible } of candidates) {
			...
			const content = ordinaryEligible ? this.pruneMessageContent(originalContent) : this.pruneRecentMessageContent(originalContent);
			...
			const entry = {
				originalSeq: seq,
				replacementSeq: session.append("tool/result", {
					...event.data,
					message
				}, {
					surfaceOp: {
