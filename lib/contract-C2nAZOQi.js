...
const TASK_STATE_SLOT_SOURCE_KIND = "task-state-slot";
/** Constant slot-channel identity recorded on every Stable task-state slot node. */
const TASK_STATE_SLOT_ID = "task-state-stable-slot";
/** Default injection token budget I when maxTokens is omitted. */
const DEFAULT_TASK_STATE_INJECTION_BUDGET_TOKENS = 512;
/** Fixed model-visible marker displayed when the stable cursor lags session eligible high-water. */
const TASK_STATE_STALE_MARKER = "[Task-state snapshot is stale; durable cursor lags eligible session events.]";
/**
...
*/
function isTaskStateSlotSource(source) {
	if (typeof source !== "object" || source === null) return false;
	const record = source;
	return record["kind"] === "task-state-slot" && record["slotId"] === "task-state-stable-slot" && typeof record["generation"] === "number" && typeof record["revision"] === "number";
}
/**
...
var TaskStateService = class extends Service {
	...
	*/
	getActiveTerminalBlock(_sessionId) {}
};
//#endregion
export { TaskStateService as a, TASK_STATE_STALE_MARKER as i, TASK_STATE_SLOT_ID as n, isTaskStateSlotSource as o, TASK_STATE_SLOT_SOURCE_KIND as r, DEFAULT_TASK_STATE_INJECTION_BUDGET_TOKENS as t };
