...
import { a as createUserMessage, c as brandNumber, i as contentHasImage, l as assertNever, o as errorChain, r as LlmError, s as freezeMessage, t as BlockAssembler, u as deepFreeze } from "./lib-DXy8Ramy.js";
...
function buildSurfaceSourceIndex(session, toolSummaryReplacementSeqs = []) {
	...
	return {
		...
		canCompactHistory(seq, minReentryTurns, allowImmediateReentry = false) {
			...
			if (allowImmediateReentry && (entry.kind === "tool-summary" || entry.kind === "tool-pruned" || entry.kind === "history-summary")) return true;
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	*/
	async compactIfNeeded(agent, trigger, signal) {
		...
		measurement = meter.measure(agent.session);
		if (measurement.totalTokens >= beforeTokens) this.logPressureStop("no-progress", measurement.totalTokens, spec.thresholdTokens);
		return result;
	}
