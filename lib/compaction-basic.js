...
function openToolGroupAudit(requestId, sessionId, group, surfaceGeneration, provider, model, fingerprint) {
	return {
		requestId,
		sessionId,
		fingerprint,
...
function completedTurnsAfter(session, replacement) {
	let count = 0;
	for (let seq = replacement.seq + 1; seq < session.seq; seq += 1) if (session.eventAt(seq)?.type === "turn/end") count += 1;
	return count;
}
//#endregion
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	transientToolGroupRetries = /* @__PURE__ */ new Set();
	toolGroupAuditStore;
	...
	*/
	async compactIfNeeded(agent, trigger, signal) {
		...
		for (let batch = 0; batch < batchLimit; batch += 1) {
			...
			if (selectedSeqs.some((seq) => roundReplacements.has(seq) || !sources.canCompactHistory(seq, policy.minReentryTurns, pressure))) break;
		...
		if (pressure && measurement.totalTokens >= spec.thresholdTokens && result !== null) throw new Error(`compaction remains above pressure after ${batchLimit} freshly-zoned forget batches (${measurement.totalTokens} >= ${spec.thresholdTokens})`);
		return result;
	}
	/** Rebuild source types from Session provenance and durable successful audits. */
	sourceIndex(session) {
		return buildSurfaceSourceIndex(session, this.toolGroupAuditStore?.recordsForSession(session.id).filter((record) => record.status === "success").flatMap((record) => record.replacementSeqs ?? []) ?? []);
	}
	...
	*/
	...
	async summarizeToolGroups(agent, target, policy, olderRange, signal, roundReplacements) {
		...
		for (const group of groups) {
			...
			if (successfulAuditFor(records, fingerprint) !== void 0) continue;
