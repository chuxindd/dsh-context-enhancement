...
const panelStyle = { position: 'fixed', zIndex: 2147483001, width: 'min(360px, calc(100vw - 24px))', padding: 14, border: '1px solid var(--dsw-border-subtle, #d9dce3)', borderRadius: 8, background: 'var(--dsw-surface-primary, #fff)', boxShadow: '0 10px 28px rgb(0 0 0 / 14%)', color: 'var(--dsw-text-primary, #1f2329)' };
...
export function ContextEnhancementAction({ sessionId, useSessions, useProjection, t }) {
    ...
    useEffect(() => {
        if (!open)
            return;
        const close = (event) => { if (event.target instanceof Node && !rootRef.current?.contains(event.target))
            setOpen(false); };
    ...
    if (agentPreset !== 'contextual')
        return null;
    return _jsxs("div", { ref: rootRef, style: rootStyle, children: [_jsxs("button", { ref: triggerRef, type: "button", style: triggerStyle, "aria-expanded": open, "aria-controls": panelId, "aria-label": t('trigger'), title: t('panelLabel'), onClick: () => { setOpen(value => !value); }, children: [_jsx("span", { style: triggerIconStyle, "aria-hidden": "true", children: "\u2726" }), t('trigger')] }), open ? _jsxs("section", { id: panelId, style: { ...panelStyle, ...panelPosition }, role: "dialog", "aria-label": t('panelLabel'), children: [_jsxs("div", { style: headingStyle, children: [_jsx("strong", { children: t('panelLabel') }), _jsx("button", { type: "button", style: closeStyle, "aria-label": t('close'), onClick: () => setOpen(false), children: "\u00D7" })] }), _jsx("p", { style: introStyle, children: t('intro') }), EFFECTS.map(effect => {
                        const count = evidence?.[effect.key] ?? 0;
                        return _jsxs("div", { style: rowStyle, children: [_jsx("span", { style: dotStyle, "aria-hidden": "true" }), _jsxs("div", { children: [_jsx("div", { children: t(effect.label) }), _jsx("div", { style: detailStyle, children: t(effect.detail) })] }), _jsxs("div", { style: { textAlign: 'right' }, children: [_jsx("div", { style: statusStyle, children: t('enabled') }), _jsx("div", { style: countStyle, children: count === 0 ? t('notTriggered') : t('evidence', { count }) })] })] }, effect.label);
