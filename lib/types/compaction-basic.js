...
import { assertNoActiveCompaction, compactSurfaceRegion, selectCompactableRange, } from "./internal/compaction/region.js";
import { partitionSurfaceZones, planForgetBatch, planPressureSpan, selectForgetBatch } from "./internal/compaction/zones.js";
import { resolveEnvelopeBudget, resolveEnvelopeZoneBudget, retainedTailFloorTokens } from "./internal/compaction/envelope-budget.js";
import { buildSurfaceSourceIndex } from "./internal/compaction/source-index.js";
import { isTaskStateSlotSource } from "./internal/task-state/contract/index.js";
import { compactionInstructionTokens, minimumCheckpointTokens, summarizeWithLlm } from "./internal/compaction/summarizer.js";
/**
 ...
 */
const TERMINAL_PRESSURE_REASONS = new Set([
    'envelope-dominated',
    'no-progress',
]);
/**
 ...
 */
async function recoverToolGroupAudits(engine, session) {
    const host = engine;
    ...
    for (const step of steps) {
        try {
            ...
            const missing = error.code === 'missing-key';
            logAuditDiagnostic(engine, `tool-group audit recovery write failed (${step.diagnostic})`
 ...
 */
function mayAttemptToolGroupSummary(engine, index, records, fingerprint) {
    ...
    if (unwritten >= TOOL_GROUP_AUDIT_MAX_ATTEMPTS) {
        logAuditDiagnosticOnce(engine, `unwritten:${fingerprint}`, `tool-group summary attempts exhausted while the audit could not record them (${AUDIT_DIAGNOSTIC.attemptsExhausted})`);
        return false;
    }
    if (!hasToolGroupAttemptBudget(records, fingerprint)) {
        logAuditDiagnosticOnce(engine, `budget:${fingerprint}`, `tool-group summary attempts exhausted (${AUDIT_DIAGNOSTIC.attemptsExhausted})`);
        return false;
    }
 ...
 */
function reportUnrecordedAuditLandings(engine, index, records) {
    ...
    for (const [, entry] of index.entries) {
        ...
        if (groupId === null || groupId === undefined)
            continue;
        if (records.some(record => record.fingerprint === groupId))
            continue;
        missing.add(groupId);
    }
    for (const groupId of missing) {
        logAuditDiagnosticOnce(engine, `unrecorded:${groupId}`, `tool-group audit has no row for a reduction the Session log proves landed (${AUDIT_DIAGNOSTIC.auditLandingUnrecorded})`);
    }
}
/**
 ...
 */
function visibleSlotTokens(session, measurement) {
 ...
 */
function reportStoppedReason(engine, logger, reason, totalTokens, thresholdTokens) {
 ...
 */
function pressureReentryVerdict(session, sources, blockerSeq) {
    const blockerEntry = sources.entry(blockerSeq);
    if (blockerEntry.kind === 'original')
        return 'new-original-content';
    if (blockerEntry.kind === 'unknown-replacement')
        return 'unknown-provenance';
    if (blockerEntry.completedTurnsAfter > 0)
        return 'completed-turns-deferred';
    ...
    if (blockerEntry.kind !== 'history-summary')
        return 'new-original-content';
...
export class BasicCompactionEngine extends CompactionEngine {
    ...
    async compactIfNeeded(agent, trigger, signal) {
        ...
        if (measurement.totalTokens < toolWatermark)
            return null;
        ...
        const roundReplacements = new Set();
        ...
        if (measurement.totalTokens < forgetWatermark)
            return null;
        ...
        if (zones.forget !== null) {
            await this.summarizeToolGroups(agent, target, policy, zones.forget, signal, roundReplacements, this.envelopeBudget(agent.session, measurement, spec).summarizerInputCapTokens);
            measurement = meter.measure(agent.session);
            zones = this.zones(agent.session, measurement, spec);
        ...
        if (!pressure) {
            ...
            while (batch < policy.maxMaintenanceBatches) {
                ...
                if (selectedSeqs.some(seq => roundReplacements.has(seq)))
                    break;
                if (selectedSeqs.some(seq => !sources.canCompactHistory(seq, policy.minReentryTurns)))
                    break;
        ...
        while (batch < policy.maxPressureBatches) {
            ...
            if (debt === 'actionable') {
                ...
                const toolDebtIdx = BasicCompactionEngine.prototype.internalFindFirstToolStageDebtIndex.call(this, agent.session, plan.range.startSeq, plan.range.endSeq, prune, budget.summarizerInputCapTokens, sources);
            ...
            for (const reason of batchReasons)
                recordVerdict(reason);
     ...
     * Resolves from the session surface and token measurement nodes.
     */
    visibleSlotTokens(session, measurement) {
     ...
     */
    inspectLedger(session, measurement, contextWindow = 128_000) {
     ...
     */
    hasPendingToolIntermediateWork(session, start, end, prune, inputCapTokens) {
        ...
        if (prune !== undefined && selected.some(seq => {
            const event = session.eventAt(seq);
            if (event?.type !== 'tool/result' || !index.isOriginalToolResult(seq))
                return false;
            ...
            const messageContent = event.data.message.content;
        ...
        // process them.
        if (!this.config.toolGroupSummarizer.enabled || this.toolGroupAuditStore === undefined)
            return 'none';
     ...
     */
    internalFindFirstToolStageDebtIndex(session, start, end, prune, inputCapTokens, index) {
        const nodes = session.surface.nodes;
        const startIndex = nodes.indexOf(start);
        const endIndex = nodes.indexOf(end);
        if (startIndex < 0 || endIndex < startIndex)
        ...
        if (prune !== undefined) {
            ...
            if (pruneDebtOffset >= 0)
                return startIndex + pruneDebtOffset;
        }
        if (!this.config?.toolGroupSummarizer?.enabled || this.toolGroupAuditStore === undefined)
            return undefined;
        const records = this.toolGroupAuditStore.recordsForSession(session.id, session.header.createdAt);
     ...
     */
    toolGroupSelectionOptions(olderRange, inputCapTokens, index, session) {
        ...
        return {
            ...
            isGroupFittable: inputCapTokens === undefined || session === undefined
                ? undefined
                : group => estimateToolGroupAuxiliaryRequestTokens(session, group, config.maxSummaryTokens, msg => this.ctx.tokenMeter.estimateMessage(msg)) <= inputCapTokens,
        };
    }
     ...
     * stop condition.
     */
    envelopeZoneBudget(session, measurement, spec) {
        const injectionTokens = visibleSlotTokens(session, measurement);
     ...
     */
    envelopeBudget(session, measurement, spec) {
        const meter = this.ctx.tokenMeter;
        const injectionTokens = visibleSlotTokens(session, measurement);
     ...
     */
    cappedMaxGroupTokens(maxGroupTokens, inputCapTokens) {
     ...
     */
    pressurePassTerminated(session, totalTokens) {
        ...
        for (const reason of memo.reasons) {
            if (TERMINAL_PRESSURE_REASONS.has(reason))
                return true;
        }
        return false;
    }
     ...
     */
    stopEnvelopeBudgetPass(session, reason, totalTokens, thresholdTokens) {
        ...
        const current = memo !== undefined
            ...
            ? memo
            : undefined;
        if (current !== undefined && current.reasons.has(reason))
            return;
        if (current !== undefined)
            current.reasons.add(reason);
        else
            this.pressureStops.set(session, { generation, surfaceNodes, totalTokens, reasons: new Set([reason]) });
        const stopReasons = this.activePressureStopReasons;
        if (stopReasons !== undefined && !stopReasons.includes(reason))
            stopReasons.push(reason);
    ...
    async recoverOverflow(agent, prune, signal) {
        ...
        for (const level of ['overflow-forget', 'overflow-tool-zone', 'overflow-recent']) {
            for (let batch = 0; batch < policy.maxPressureBatches; batch += 1) {
                const current = this.ctx.tokenMeter.measure(session);
                if (current.totalTokens < spec.thresholdTokens)
                    return latest;
                ...
                if (selectedSeqs.some(seq => !sourceIndex.canCompactHistory(seq, policy.minReentryTurns)))
                    break;
    ...
    async summarizeToolGroups(agent, target, policy, olderRange, signal, roundReplacements, inputCapTokens) {
        const config = this.config.toolGroupSummarizer;
        ...
        const groups = selectToolGroups(session, this.toolGroupSelectionOptions({ start: olderRange.startSeq, end: olderRange.endSeq }, inputCapTokens, selectionSources, session));
        ...
        for (const group of groups) {
            ...
            const sources = this.sourceIndex(session);
            if (group.toolResultSeqs.some(seq => !sources.isOriginalToolResult(seq)))
                continue;
            ...
            if (!mayAttemptToolGroupSummary(this, sources, records, fingerprint))
                continue;
            ...
            const open = slot === undefined
                ? openToolGroupAudit(`tg-${randomUUID()}`, session.id, group, session.surface.replaceGeneration, policy.summarizationProvider || target.provider, policy.summarizationModel || target.model, fingerprint, { createdAt: session.header.createdAt }, { ownerId: auditOwnerId(this) })
                : slot.adopted
            ...
            try {
                ...
                logAuditDiagnostic(this, `tool-group summary audit write failed (${AUDIT_DIAGNOSTIC.auditWriteFailed}); `
            ...
            try {
                ...
                if (!auditWrites) {
                    ...
                    // terminal row exists.
                    logAuditDiagnostic(this, `tool-group summary attempt failed without an audit row (${AUDIT_DIAGNOSTIC.auditWriteFailed}): `
                        + `${error instanceof Error ? error.message : String(error)}`);
                    continue;
                }
            ...
            if (!auditWrites) {
                ...
                logAuditDiagnostic(this, `tool-group summary audit commit skipped (${AUDIT_DIAGNOSTIC.auditWriteFailed}); `
     ...
     */
    async commitToolGroupSuccess(store, requestId, patch) {
        for (let attempt = 0; attempt < 2; attempt += 1) {
            try {
                ...
                if (attempt === 0)
                    continue;
                this.ctx.logger.warn(`tool-group summary audit success commit failed: ${error instanceof Error ? error.message : String(error)}; `
        ...
        try {
            ...
            this.ctx.logger.warn(`tool-group summary audit open-record fallback persistence failed: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`);
        }
    }
    /**
