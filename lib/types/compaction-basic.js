...
import { partitionSurfaceZones, selectForgetBatch } from "./internal/compaction/zones.js";
...
export class BasicCompactionEngine extends CompactionEngine {
    ...
    transientToolGroupRetries = new Set();
    toolGroupAuditStore;
    ...
    async compactIfNeeded(agent, trigger, signal) {
        ...
        for (let batch = 0; batch < batchLimit; batch += 1) {
            ...
            if (selectedSeqs.some(seq => roundReplacements.has(seq)
                || !sources.canCompactHistory(seq, policy.minReentryTurns, pressure)))
                break;
    ...
    async summarizeToolGroups(agent, target, policy, olderRange, signal, roundReplacements) {
        ...
        for (const group of groups) {
            ...
            if (settled?.status === 'fallback')
                continue;
            if (settled?.status === 'failure' && this.transientToolGroupRetries.has(fingerprint))
                continue;
            ...
            try {
                ...
                if (transient)
                    this.transientToolGroupRetries.add(fingerprint);
                try {
