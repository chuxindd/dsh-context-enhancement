...
function rejectDuplicateIds(value, ctx) {
    const seen = new Map();
    for (const key of ['facts', 'decisions', 'constraints', 'risks']) {
 ...
 */
export function terminalGeneration(base, maxInputBytes) {
    return {
 ...
 */
export function sameTerminalGeneration(left, right) {
 ...
 */
export function activeTerminalBlock(verdict, current) {
    if (verdict === undefined || verdict.kind === 'quarantined')
        return undefined;
