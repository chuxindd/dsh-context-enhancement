...
export { classifyAuditRow, deriveAuditTimeline, finishAuditRow, highestCertifiedRevision, openAuditRow, rowsForLifecycle, selectRepairRow, taskStateAuditSchema, } from "./audit.js";
/** Message source kind of a plugin-owned model-visible Stable task-state slot node. */
export const TASK_STATE_SLOT_SOURCE_KIND = 'task-state-slot';
/** Constant slot-channel identity recorded on every Stable task-state slot node. */
export const TASK_STATE_SLOT_ID = 'task-state-stable-slot';
/** Default injection token budget I when maxTokens is omitted. */
export const DEFAULT_TASK_STATE_INJECTION_BUDGET_TOKENS = 512;
/** Fixed model-visible marker displayed when the stable cursor lags session eligible high-water. */
export const TASK_STATE_STALE_MARKER = '[Task-state snapshot is stale; durable cursor lags eligible session events.]';
/**
 ...
 */
export function isTaskStateSlotSource(source) {
    if (typeof source !== 'object' || source === null)
        return false;
    const record = source;
    return record['kind'] === TASK_STATE_SLOT_SOURCE_KIND
        && record['slotId'] === TASK_STATE_SLOT_ID
