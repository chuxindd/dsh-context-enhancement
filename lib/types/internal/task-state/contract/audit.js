 ...
 */
import { z } from 'zod';
import { taskStateInheritedPrefixSchema } from "./spec.js";
...
export const taskStateAuditOpenSchema = z.object({
    ...
    request: z.object({
        ...
        // The inherited fork boundary this window was bounded by. Absent on every
        // row of a lifecycle that began on its own events, which is the only shape
        // such a lifecycle can produce.
...
export const taskStateAuditFinishedSchema = z.object({
    outcome: z.enum(['success', 'failure', 'manual', 'repair', 'terminal-infeasible', 'aborted']),
 ...
 */
export function openAuditRow(requestId, session, request, time = Date.now()) {
 ...
 */
export function finishAuditRow(row, finished) {
...
export function classifyAuditRow(row) {
    ...
    if (outcome === 'success' || outcome === 'manual' || outcome === 'repair') {
        return 'committed';
    }
    if (outcome === 'terminal-infeasible') {
        return 'terminal-infeasible';
    }
    if (outcome === 'aborted') {
        return 'aborted';
    }
    const code = row.finished.error?.code;
    if (code === 'ABORTED' || code === 'SERVICE_DISPOSED') {
        return 'aborted';
    }
    if (code === 'BUDGET') {
        return 'terminal-infeasible';
 ...
 */
export function deriveAuditTimeline(rows) {
    return [...rows]
 ...
 */
export function highestCertifiedRevision(rows) {
    let latest = 0;
    for (const row of rows) {
        const finished = row.finished;
        if (finished === undefined)
            continue;
        if (finished.outcome !== 'success' && finished.outcome !== 'manual' && finished.outcome !== 'repair')
            continue;
        if (finished.revision > latest)
            latest = finished.revision;
    }
    return latest;
}
/**
 ...
 */
export function selectRepairRow(rows, committedRevision) {
    let newest;
    for (const row of rows) {
        if (row.finished !== undefined || row.request.revision !== committedRevision)
            continue;
 ...
 */
export function rowsForLifecycle(rows, identity) {
    return rows.filter(row => row.session.createdAt === identity.createdAt
