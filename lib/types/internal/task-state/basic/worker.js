...
import { TaskStateRequestId } from "../contract/brand.js";
import { terminalGeneration } from "../contract/spec.js";
import { foldBatchWindow } from "./batch.js";
import { TASK_STATE_FILTER_VERSION, filterEvent, isEligibleType } from "./filter.js";
import { isAuthorityEventType } from "./authority.js";
import { TASK_STATE_INPUT_SCHEMA_VERSION } from "./prompt.js";
import { sessionInheritedPrefix } from "./inherited.js";
...
export class TaskStateWorker {
    ...
    maybeScheduleStartup() {
        if (!this.isOpen || this.startupScheduled)
            return;
    ...
    settleCycle(kind, outcome) {
        ...
        if (kind === 'trailing' || kind === 'urgent') {
            ...
            if (urgentOutstanding && this.pendingEligible >= 1)
                this.launch('urgent');
            return;
        }
        ...
        if (wantTrailing && this.pendingEligible >= 1)
            this.launch(urgentOutstanding ? 'urgent' : 'trailing');
    }
    /**
    ...
    async handleInfeasible(kind, session, base, cursor, folded) {
        ...
        this.ctx.logger.warn(`task-state-basic: ${this.sessionId} quarantined infeasible event seq ${quarantinedSeq} `
