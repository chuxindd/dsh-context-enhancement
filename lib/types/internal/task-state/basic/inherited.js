 ...
 */
export function sessionInheritedPrefix(session) {
    const ownBoundarySeq = Number(session.inheritedEventCount);
    if (!Number.isSafeInteger(ownBoundarySeq) || ownBoundarySeq <= 0)
        return null;
 ...
 */
export function inheritedCursorFloor(prefix) {
 ...
 */
export function inheritedCoverageRefused(prefix, marker) {
    if (marker === undefined)
        return false;
    if (prefix === null)
        return true;
 ...
 */
export function isInheritedSeq(prefix, seq) {
