 ...
 */
import { MARKER_BYTES, boundUtf8 } from "./bytes.js";
/** The absent Goal view: no authority fact ever established one. */
export const NO_GOAL_VIEW = Object.freeze({ status: 'none' });
/** The absent TODO view: no authority fact ever established one. */
export const NO_TODO_VIEW = Object.freeze({ status: 'none', items: Object.freeze([]) });
/** The authority fact types whose observation is urgent (never waits for `minEvents`). */
const AUTHORITY_EVENT_TYPES = new Set(['goal/change', 'todo/write']);
/**
 ...
 */
export function isAuthorityEventType(type) {
...
function todoViewOf(event) {
    ...
    if (cleared)
        return { status: 'cleared', sourceSeq: event.seq, items: [] };
    if (raw === undefined)
        return undefined;
    const items = [];
...
function sameTodoView(left, right) {
    if (left.status !== right.status || left.items.length !== right.items.length)
        return false;
    for (let index = 0; index < left.items.length; index += 1) {
        ...
        if (a === undefined || b === undefined)
            return false;
        if (a.content !== b.content || a.status !== b.status)
            return false;
    }
    return true;
}
 ...
 */
export function todoReferencesOf(view, maxEntryBytes) {
    if (view.status !== 'current' || view.sourceSeq === undefined)
        return [];
 ...
 */
export function resolveAuthorityViews(events, base, limits) {
    ...
    if (todoChanged)
        changed.push('todo');
    if (goalChanged && resolvedGoal.status === 'cleared' && baseGoal.status === 'current')
        cleared.push('goal');
    if (todoChanged && resolvedTodo.status === 'cleared' && baseTodo.status === 'current')
        cleared.push('todo');
    return {
 ...
 */
export function authorityUrgency(event, base) {
