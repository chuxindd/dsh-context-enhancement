...
import { DEFAULT_TASK_STATE_INJECTION_BUDGET_TOKENS, TASK_STATE_SLOT_ID, TASK_STATE_SLOT_SOURCE_KIND, isTaskStateSlotSource, } from "../contract/index.js";
import { isEligibleType } from "../basic/filter.js";
import { renderTaskStateSlot, resolveInjectionBudget, } from "./render.js";
export { TASK_STATE_STALE_MARKER, TASK_STATE_TRUNCATION_MARKER, estimateSlotTokens, renderTaskStateSlot, renderTaskStateSnapshot, resolveInjectionBudget, } from "./render.js";
...
export { DEFAULT_TASK_STATE_INJECTION_BUDGET_TOKENS, TASK_STATE_SLOT_ID, TASK_STATE_SLOT_SOURCE_KIND, isTaskStateSlotSource, } from "../contract/index.js";
 ...
 * leaves none behind.
 */
const slotRegistries = new WeakMap();
/**
 ...
 */
export function taskStateSlotDiagnostics(ctx) {
 ...
 */
export function isOwnSlotNode(session, source) {
...
function nextGeneration(session, floor) {
    ...
    for (const event of session.snapshotEvents()) {
        if (event.type !== 'user/message' || !isTaskStateSlotSource(event.data.source))
            continue;
        if (event.data.source.generation > highest)
            highest = event.data.source.generation;
    }
    return highest + 1;
}
...
function maintainSlot(ctx, slots, session, budget) {
    ...
    if (store !== undefined && store.get(session.id) !== session)
        return;
...
export function apply(ctx, config) {
    ...
    const slots = new Map();
    const root = ctx.root;
    const registries = slotRegistries.get(root) ?? new Set();
