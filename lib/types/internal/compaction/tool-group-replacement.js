...
import { contentDigest, toolGroupFingerprint } from "./tool-group-audit.js";
import { reductionProvenance, shadowPriceWithProvenance } from "./source-index.js";
import { toolResultTextLength } from "./tool-groups.js";
/**
 ...
 */
export function replaceToolGroup(session, group, summary, options) {
    ...
    for (const { sourceSeq, event, item } of snapshot) {
        ...
        if (toolResultTextLength(original.content) === 0)
            continue;
        const text = {
            type: 'text',
        ...
        const content = original.content.map((messageBlock, index) => {
            ...
            if (messageBlock.type !== 'tool-result')
                return messageBlock;
            if (index === blockIndex) {
        ...
        if (options.estimateTokens(message) >= shadowedTokens)
            continue;
 ...
 */
function toolGroupIdentity(session, group) {
