import { toolPairingBalancedBefore } from "./tool-pairing.js";
import { codePointLength } from "./pruner-config.js";
...
export function selectToolGroups(session, options = {}) {
    ...
    for (const segment of toolSegments(session)) {
        ...
        if (chars < minGroupChars || estimatedTokens < minGroupTokens)
            continue;
...
function buildGroup(session, seqs, thresholds, isGroupFittable) {
    ...
    if (toolResultSeqs.length < thresholds.minGroupResults)
        return null;
    const estimatedTokens = events.reduce((total, event) => total + thresholds.estimateTokens(event), 0);
    if (estimatedTokens < thresholds.minGroupTokens || estimatedTokens > thresholds.maxGroupTokens)
        return null;
    const chars = events.reduce((total, event) => total + (event.type === 'tool/result' ? thresholds.measureText(event) : 0), 0);
    if (chars < thresholds.minGroupChars)
        return null;
...
function splitOversizedRun(session, seqs, maxGroupTokens, estimateTokens) {
    ...
    for (let position = 1; position < seqs.length; position += 1) {
        if (safeChunkCut(session, seqs[position - 1], seqs[position]))
            safeCuts.add(position);
    }
    const chunks = [];
    let start = 0;
    while (start < seqs.length) {
        ...
        for (let end = start; end < seqs.length; end += 1) {
            running += tokens[end];
            if (running > maxGroupTokens)
                break;
            if (end + 1 === seqs.length || safeCuts.has(end + 1))
                fitEnd = end + 1;
        }
        if (fitEnd > start) {
...
function safeChunkCut(session, previous, next) {
    if (!toolPairingBalancedBefore(session, next))
        return false;
 ...
 */
export function toolResultTextLength(content) {
    let chars = 0;
