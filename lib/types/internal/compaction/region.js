...
import { CompactionId, ManualCompactionError, compactCheckpointSource, } from '@deepseek-ai/dsh-compaction';
import { createUserMessage, errorChain } from '@deepseek-ai/dsh-llm';
import { SessionSeq } from '@deepseek-ai/dsh-session';
import { isIsolatedOldSummaryRange } from "./selection-guard.js";
 ...
 */
export function splitRetainedTail(session, measurement, retainTokens) {
    const pricedNodes = measurement.nodes;
    if (pricedNodes.length === 0) {
        return { keepFromIdx: 0, cutoffSeq: undefined, olderRange: null, olderSeqs: [] };
    }
    ...
    // keepFromIdx < surfaceNodes.length holds after the loop, so the older head
    // seqs (0..keepFromIdx-1) are non-empty exactly when a head exists.
    const olderSeqs = surfaceNodes.slice(0, keepFromIdx);
    const olderStart = olderSeqs.length === 0 ? undefined : olderSeqs[0];
    const olderEnd = olderSeqs.length === 0 ? undefined : olderSeqs.at(-1);
    return {
        keepFromIdx,
        cutoffSeq: olderEnd,
        olderRange: olderStart === undefined || olderEnd === undefined
            ? null
 ...
 */
export function selectCompactableRange(session, measurement, retainTokens, guardIsolatedOldSummary = true) {
    const split = splitRetainedTail(session, measurement, retainTokens);
    if (split.cutoffSeq === undefined)
        return null;
    if (guardIsolatedOldSummary && isIsolatedOldSummaryRange(session, split.olderSeqs))
        return null;
    const first = session.surface.nodes[0];
    return { start: first, end: split.cutoffSeq };
}
/**
...
function findMatchingCompactionStart(session, endSeq, compactionId) {
    for (let seq = Number(endSeq) - 1; seq >= 0; seq -= 1) {
        ...
        if (event?.type === 'compaction/start' && event.data.compactionId === compactionId)
            return event;
    }
    return undefined;
}
