...
function isToolBurstNode(event) {
    if (event.type === 'assistant/message')
        return isToolCallAssistantMessage(event);
 ...
 */
export function toolSegments(session) {
    ...
    while (index < surface.length) {
        ...
        while (runEnd < surface.length) {
            ...
            if (!isToolBurstNode(candidate) || turnOf(candidate) !== turn)
                break;
            runEnd += 1;
        }
        ...
        for (let cursor = index; cursor < runEnd; cursor += 1) {
            ...
            if (firstBalancedIndex === -1 && toolPairingBalancedBefore(session, seq))
                firstBalancedIndex = cursor;
            if (toolPairingBalancedAfter(session, seq))
                lastBalancedIndex = cursor;
        }
 ...
 */
export function toolSegmentAt(session, seq) {
    if (!session.surface.nodes.includes(seq)) {
        throw new Error(`tool-segments: surface seq ${seq} not found`);
    }
    return toolSegments(session).find(segment => segment.seqs.includes(seq)) ?? null;
}
//# sourceMappingURL=tool-segments.js.map