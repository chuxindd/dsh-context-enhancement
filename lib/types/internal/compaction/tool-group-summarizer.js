...
import { buildToolGroupSummaryInput, parseToolGroupSummary } from "./tool-group-summary.js";
...
export function buildToolGroupSummaryMessage(session, group) {
...
export function estimateToolGroupAuxiliaryRequestTokens(session, group, reserveTokens, estimateMessage) {
    const message = buildToolGroupSummaryMessage(session, group);
    return estimateMessage(message) + Math.max(0, reserveTokens);
}
export async function summarizeToolGroup(ctx, session, group, agent, route, signal, inputCapTokens) {
    ...
    if (inputCapTokens !== undefined) {
        const fullTokens = estimateToolGroupAuxiliaryRequestTokens(session, group, route.maxTokens, msg => ctx.tokenMeter.estimateMessage(msg));
        if (fullTokens > inputCapTokens) {
            throw new ToolGroupSummaryFallbackError('stream', `tool-group-summary: complete auxiliary request (${fullTokens} tokens) exceeds input cap (${inputCapTokens} tokens)`);
        }
    }
    const input = buildToolGroupSummaryInput(session, group);
    ...
    try {
        ...
        const text = rawOutput.filter((block) => block.type === 'text')
            .map(block => block.text).join('');
        if (text.trim().length === 0)
            throw new ToolGroupSummaryFallbackError('empty', 'tool-group-summary: model returned no text');
        let parsed;
        try {
        ...
        if (error instanceof ToolGroupSummaryFallbackError)
            throw error;
...
//# sourceMappingURL=tool-group-summarizer.js.map