import { r as TypertRemoteService, t as Remote } from "./lib-DkAdRvjl.js";
//#region lib/types/internal/task-state/control/service.js
...
var FrameQueue = class {
	...
	next(signal) {
		if (signal.aborted) return Promise.reject(signal.reason ?? /* @__PURE__ */ new Error("task-state stream aborted"));
		if (this.frames.length > 0) return Promise.resolve({
			value: this.frames.shift(),
			done: false
		});
		...
		return new Promise((resolve, reject) => {
			const abort = () => {
				...
				reject(signal.reason ?? /* @__PURE__ */ new Error("task-state stream aborted"));
			};
...
let TaskStateControlService = (() => {
	...
	return class TaskStateControlService extends _classSuper {
		...
		constructor(ctx) {
			...
			ctx.effect(() => {
				const dispose = ctx.taskState.subscribeCommitted?.((sessionId, stable) => {
		...
		queues = (__runInitializers(this, _instanceExtraInitializers), /* @__PURE__ */ new Set());
		control(signal) {
		...
		edit(request) {
			...
			if (source.editStable === void 0) return Promise.resolve({
				ok: false,
				code: "unavailable",
				message: "This task-state provider does not support editing."
			});
		...
		async *openControlStream(signal) {
			...
			try {
				const items = {};
				for (const session of this.ctx.sessions.list()) items[session.id] = this.ctx.taskState.getStable(session.id) ?? null;
				if (signal.aborted) return;
				yield {
					type: "baseline",
					value: { items }
...
//#endregion
export { TaskStateControlService as default };
